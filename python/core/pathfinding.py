...
def compute_cost_matrix(base_cost, slope_x, slope_y, nodes, ds=8,
                        cache_enabled=True, cell_m=1.0):
    """Build asymmetric cost matrix via anisotropic Dijkstra from each node.

    Parameters
    ----------
    cell_m : float
        Real-world metres per pixel of `base_cost`/`slope_x`/`slope_y`
        (i.e. the original raster resolution, before downsampling by
        `ds`). Needed to convert grid-step elevation/distance deltas into
        real metres for gain/loss/dist -- see `_compute_cost_matrix_python`.
    ...
    dist_m : (n, n) ndarray
        Cumulative horizontal path length (metres) along the same path.
        Used as the distance term (weighted by mu) in the fatigue model.
    """
    h, w = base_cost.shape
    h_ds, w_ds = h // ds, w // ds
    ...
    step_m = ds * cell_m  # real metres covered by one downsampled grid step
    ...
    if cache_enabled and os.path.exists(cache_file):
        with np.load(cache_file) as data:
            cm = data['cm'].copy()
            # Backward compat: older caches won't have gain/loss/dist saved.
            if 'gain' in data and 'loss' in data and 'dist' in data:
                gain_m = data['gain'].copy()
                loss_m = data['loss'].copy()
                dist_m = data['dist'].copy()
                print(f"    ✅ Loaded cached cost matrix ({cache_file})")
                return cm, gain_m, loss_m, dist_m
        print(f"    ⚠️ Cache {cache_file} missing gain/loss/dist, recomputing")

    print(f"    DS grid: {w_ds}x{h_ds}, {n} nodes, ds={ds}")

    if NUMBA_AVAILABLE:
        cm, gain_m, loss_m, dist_m = _compute_cost_matrix_numba(ds_base, ds_sx, ds_sy, dn, n, h_ds, w_ds, step_m)
    else:
        cm, gain_m, loss_m, dist_m = _compute_cost_matrix_python(ds_base, ds_sx, ds_sy, dn, n, h_ds, w_ds, step_m)

    np.savez_compressed(cache_file, cm=cm, gain=gain_m, loss=loss_m, dist=dist_m)
    print(f"    💾 Saved cost matrix cache ({cache_file})")
    return cm, gain_m, loss_m, dist_m


def _compute_cost_matrix_python(ds_base, ds_sx, ds_sy, dn, n, h_ds, w_ds, step_m):
    """Pure Python fallback for cost matrix computation.

    Alongside the Minetti-weighted cost `dist` (internal cost units, not to
    be confused with the returned `dist_m`), this also tracks three
    parallel accumulators along the *same* shortest-cost tree:
      ...
      - `pathlen`: cumulative real-world path length (metres)
    These are exported as phi_plus / phi_minus / dist arc weights for the
    ...
    `step_m` is the real-world metres covered by one grid step in this
    (already downsampled) grid -- i.e. ds * original cell size. Elevation
    deltas use `dir_slope * step_m` (real metres), not `dir_slope *
    step_len` (grid-step count): step_len alone is a dimensionless step
    multiplier (1.0 orthogonal, 1.414 diagonal) and needs the step_m
    factor to become an actual elevation change in metres.
    """
    import heapq
        ...
        while pq:
            ...
            for k in range(8):
                ...
                if not np.isfinite(bc):
                    continue
                step_len = nsl[k]
                step_real_m = step_len * step_m  # real metres covered by this step
                ...
                dz = dir_slope * step_real_m  # real elevation delta over the step
                if bc >= 9.0:
                    nd = d + step_len * 10.0
                else:
                ...
                if nd < dist[ny_, nx_]:
                    ...
                    pathlen[ny_, nx_] = pathlen[y, x] + step_real_m
...
def _compute_cost_matrix_numba(ds_base, ds_sx, ds_sy, dn, n, h_ds, w_ds, step_m):
    """Numba-accelerated cost matrix computation.

    `step_m`: real-world metres covered by one grid step in this
    (already downsampled) grid -- see `_compute_cost_matrix_python`.
    """
    from numba import njit
    ...
    @njit(cache=True)
    def _dijkstra_nb(base_cost, slope_x, slope_y, sx, sy, step_m):
        h, w = base_cost.shape
        INF = 1e18
        ...
        pathlen = np.zeros((h, w), dtype=np.float64)  # cumulative real path length metres (cost-optimal path)
        dist[sy, sx] = 0.0
        ...
        while hn > 0:
            ...
            for k in range(8):
                ...
                dz = ds * step_real_m  # signed elevation delta over this step (real m)
                if bc >= 9.0:
                    nd = dd + step_len * 10.0
                else:
        ...
        return dist, gain, loss, pathlen

    # JIT warmup
    print("    JIT compiling (first run only)...")
    t0 = time_module.time()
    _dijkstra_nb(ds_base[:10, :10], ds_sx[:10, :10], ds_sy[:10, :10], 0, 0, step_m)
    ...
    for i in range(n):
        ...
        sx = min(max(dn[i][0], 0), w_ds - 1)
        sy = min(max(dn[i][1], 0), h_ds - 1)
        dist, gain, loss, pathlen = _dijkstra_nb(ds_base, ds_sx, ds_sy, sx, sy, step_m)
        for j in range(n):
            if i != j:
                tx = min(max(dn[j][0], 0), w_ds - 1)
                ty = min(max(dn[j][1], 0), h_ds - 1)
