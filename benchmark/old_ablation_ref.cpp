...
// ΓöÇΓöÇ Global ablation flags ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
struct LPModel {
    ...
    // ΓöÇΓöÇ build (flow-based formulation) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    //
    // Variables:
    //   x_ij Γêê {0,1}  ΓÇö arc traversal
    //   y_i  Γêê {0,1}  ΓÇö node visit
    //   f_ij ΓëÑ 0      ΓÇö cumulative elapsed time flowing along arc (i,j)
    //
    ...
    //   t_j = Γêæ_i f_ij + C_ij ┬╖ x_ij  (for visited j)
    //   equivalently: Γêæ_i (f_ij + C_ij ┬╖ x_ij) = Γêæ_k f_jk  (time in = time out)
    //
    // Fatigue budget (LINEAR ΓÇö no McCormick needed):
    //   Γêæ C_ij ┬╖ x_ij + (╬╗/B) ┬╖ Γêæ C_ij ┬╖ f_ij Γëñ B
    //
    ...
    void build(const Input& inp) {
        ...
        // x[i][j] ΓÇö arc variables with structural elimination
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
        ...
        // y[i] ΓÇö node visit variables (objective coefficients = scores)
        for (int i = 0; i < n; ++i)
            y_col[i] = add_col(0.0, 1.0, inp.pts[i]);
        fix_col(y_col[0], 1.0);

        // f[i][j] ΓÇö flow variables: f_ij carries arrival time at i along arc (i,j)
        //   0 Γëñ f_ij Γëñ B ┬╖ x_ij (enforced via coupling constraints below)
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
...
// ΓöÇΓöÇ Max-flow separation (push-relabel) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Hungarian algorithm (assignment relaxation) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Returns the minimum cost assignment on an n├ùn cost matrix.
...
// ΓöÇΓöÇ Routing Infeasibility Cuts ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// For a subset S of nodes, if the assignment relaxation lower bound on the
// cost of any cycle through S Γê¬ {depot} exceeds the budget B, then
...
// ΓöÇΓöÇ Cycle Cover Cuts (directed adaptation of Fischetti 1998) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Path Inequality Cuts (directed adaptation of Fischetti 1998) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
struct Solver {
    ...
    // Ablation flags ΓÇö toggle cut families on/off
