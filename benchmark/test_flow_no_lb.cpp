...
struct LPModel {
    ...
    void add_time_flow_coupling(const std::vector<std::vector<double>>& cm, double bud_raw) {
        // LOOSE coupling: f[i][j] <= B * x[i][j]  (original v2 formulation)
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
...
int find_and_add_cover_cuts(LPModel& lp, const Input& inp, int max_covers = 3) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j) {
            ...
            // Fatigue-aware weight DISABLED — using plain weight (original v2)
...
struct Solver {
    ...
    void process_node(BNCNode node, std::stack<BNCNode>& node_stack) {
        ...
        for (int cut_iter = 0; cut_iter < max_cuts; ++cut_iter) {
            ...
            // Routing infeasibility cuts DISABLED for testing
            int routing_cuts = 0;
